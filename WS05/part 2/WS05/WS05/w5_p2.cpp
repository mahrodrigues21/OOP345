...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		while (cnt < 4 && std::getline(fileIn, tempFile)){
			if (tempFile[0] != '#'){
				Book tempBook(tempFile);
				library += tempBook;
				cnt++;
			}
		}
		...
		while (std::getline(fileIn, tempFile)){
			if (tempFile[0] != '#'){
				Book tempBook(tempFile);
				library += tempBook;
				cnt++;
	...
	if (argc > 2) {
		...
		while (cnt < 5 && std::getline(fileIn, tempFile)){
			if (tempFile[0] != '#' && cnt < 5){
				Movie tempMovie(tempFile);
				movies[cnt++] =  tempMovie;
			}
		}
	}
